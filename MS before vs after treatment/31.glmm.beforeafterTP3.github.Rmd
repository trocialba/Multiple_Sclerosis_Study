...
```{r}
#filter taxa>50
#ps_ST = filter_taxa(ps_ST2, function(x) sum(x) > 50, TRUE)
ps_ST <- ps_ST2
...
1.Sample Table: 
Separate sample_data, ocrevus treatment only, in two groups: Before treatment 
(TP0,TP1) and After treatment (TP3,TP4)
...
t <- subset(t1, TimePoint %in% c("TP1", "TP3"))
...
```{r}
ps14 <- subset_samples(ps_ST, TimePoint %in% c("TP1", "TP3"))
ps15 <- psmelt(ps14)
```

```{r}
ps.melt14 <- ps15[apply(ps15!=0, 1, all),]
#export(ps.melt14, "MS-O.asv.melt.all.13.xlsx")
```
...
```{r}
export(treat_p_tax, "MS.stool.diff.abundance.oc.TP1.TP3.xlsx")
```
...
8. log fold change is in coeficients of glm 
https://genomebiology.biomedcentral.com/articles/10.1186/s13059-014-0550-8

9. log fold change
https://stats.stackexchange.com/questions/230484/how-to-calculate-fold-change-when-we-have-replicate